...
def test_projectile_range_matches_the_closed_form():
    ...
    got = nm.classical.projectile_range(speed, angle, g)
    ...
    for other in (30.0, 60.0, 44.0, 46.0):
        assert nm.classical.projectile_range(speed, math.radians(other), g) < got
...
def test_arguments_are_not_silently_reordered():
    ...
    assert nm.classical.displacement(3.0, 2.0, 4.0) == pytest.approx(3 * 4 + 0.5 * 2 * 16)


def test_orbital_and_escape_velocities_are_related_by_root_two():
    m, r = 5.972e24, 6.371e6
    v_orbit = nm.gravitation.orbital_velocity(m, r)
    v_escape = nm.gravitation.escape_velocity(m, r)
...
def test_a_pure_tone_lands_in_one_bin():
    ...
    mags = [abs(z) for z in nm.transforms.fft.fft(signal)]
...
def test_descriptive_statistics_on_a_known_sample():
    ...
    assert nm.statistics.descriptive.mean(data) == pytest.approx(5.0)
    # Population variance of this textbook sample is exactly 4.
    assert nm.statistics.descriptive.variance(data) == pytest.approx(4.0)
    assert nm.statistics.descriptive.std_deviation(data) == pytest.approx(2.0)


def test_gamma_of_a_half_integer_is_root_pi():
    assert nm.special.gamma.gamma(0.5) == pytest.approx(math.sqrt(math.pi))
    assert nm.special.gamma.gamma(5.0) == pytest.approx(24.0)
...
def test_a_long_vector_survives_the_crossing_intact():
    ...
    out = nm.statistics.descriptive.mean(data)
...
def test_geodesy_reaches_an_associated_constant_and_a_tuple_return():
    ...
    wgs84 = nm.geometry.geodesy.Ellipsoid.WGS84
    ...
    dist, az_fwd, az_rev = nm.geometry.geodesy.vincenty_inverse(
    ...
    same, _, _ = nm.geometry.geodesy.vincenty_inverse(
        math.radians(51.5074), math.radians(-0.1278),
        math.radians(40.7128), math.radians(-74.0060),
