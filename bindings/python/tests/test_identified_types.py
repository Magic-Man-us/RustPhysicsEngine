...
def test_complex_goes_both_ways_as_the_builtin():
    spectrum = rpe.transforms.fft.fft([1.0, 2.0, 3.0, 4.0])
...
def test_bigint_is_a_python_int_of_any_size():
    ...
    big = rpe.exact.bigint.factorial(100)
    import math
...
def test_a_huge_python_int_survives_the_trip_in_and_out():
    ...
    assert rpe.exact.bigint.gcd(n, n) == n
    assert rpe.exact.bigint.gcd(-n, n) == n


def test_modular_exponentiation_agrees_with_python():
    base, exp, mod = 3, 10**40 + 7, 2**61 - 1
    assert rpe.exact.bigint.mod_pow(base, exp, mod) == pow(base, exp, mod)


def test_rational_is_a_fraction_and_stays_exact():
    third = Fraction(1, 3)
    doubled = rpe.exact.rational.add(third, third)
    ...
    tenth = rpe.exact.rational.add(Fraction(1, 20), Fraction(1, 20))
    assert tenth == Fraction(1, 10)
    assert tenth != 0.1


def test_a_plain_int_is_a_rational():
    assert rpe.exact.rational.add(2, Fraction(1, 2)) == Fraction(5, 2)
...
def test_a_float_is_refused_where_a_rational_is_expected():
    ...
    with pytest.raises(TypeError):
        rpe.exact.rational.add(0.1, Fraction(1, 10))


def test_continued_fractions_round_trip():
    cf = rpe.exact.rational.to_continued_fraction(Fraction(415, 93))
    assert cf == [4, 2, 6, 7]
