...
def test_a_singular_matrix_raises_rather_than_returning_nonsense():
    ...
    with pytest.raises(rpe.SingularMatrixError) as excinfo:
        rpe.linalg.lu.solve(singular, [1.0, 2.0])
    assert "singular" in str(excinfo.value)
    # And it is catchable at every level above it.
    with pytest.raises(rpe.SolverError):
        rpe.linalg.lu.solve(singular, [1.0, 2.0])
    with pytest.raises(rpe.PhysicsError):
        rpe.linalg.lu.solve(singular, [1.0, 2.0])


def test_a_shape_mismatch_carries_the_two_shapes():
    with pytest.raises(rpe.DimensionMismatchError) as excinfo:
        rpe.linalg.lu.solve([[1.0, 2.0], [3.0, 4.0]], [1.0, 2.0, 3.0])
    err = excinfo.value
...
def test_an_assertion_in_the_library_becomes_an_exception_with_its_message():
    ...
    wants.
    """
    with pytest.raises(rpe.InvalidArgumentError) as excinfo:
...
def test_a_panic_does_not_leave_the_next_call_broken():
    with pytest.raises(rpe.InvalidArgumentError):
        rpe.classical.acceleration(force=10.0, mass=0.0)
    # The guard has to reset its state, or every later call inherits it.
    assert rpe.classical.acceleration(force=10.0, mass=2.0) == 5.0
...
def test_the_integrand_error_wins_over_whatever_it_caused():
    ...
    with pytest.raises(ValueError, match="mine"):
        rpe.numerical.integrate.simpson(bad, 0.0, 1.0, 1000)
...
def test_a_geometry_failure_maps_to_the_geometry_branch():
    with pytest.raises(rpe.EmptyInputError):
        # A triangulation needs at least three points.
        rpe.fem.fem2d.FemMesh2.from_delaunay([(0.0, 0.0), (1.0, 0.0)])
    with pytest.raises(rpe.DegenerateGeometryError) as excinfo:
        # Three collinear points enclose no area.
        rpe.fem.fem2d.FemMesh2.from_delaunay([(0.0, 0.0), (1.0, 0.0), (2.0, 0.0)])
...
def test_wrong_python_types_are_TypeError_not_a_crash():
    with pytest.raises(TypeError):
        rpe.classical.displacement("fast", 1.0, 1.0)
    with pytest.raises(TypeError):
        rpe.statistics.descriptive.mean("abc")
