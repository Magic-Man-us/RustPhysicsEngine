...
def test_chaining_in_one_expression_keeps_every_gate():
    c = rpe.quantum.circuit.Circuit(3)
    c.h(0).cx(0, 1).cx(1, 2)
    state = c.run(rpe.quantum.circuit.QState.zero(3))
    probabilities = [round(abs(z) ** 2, 6) for z in state.amps]
...
def test_a_mutable_slice_of_objects_is_written_back():
    ...
    points = [
        rpe.math.Vec2(3.0, 3.0),
        rpe.math.Vec2(0.0, 0.0),
        rpe.math.Vec2(1.0, 1.0),
    ]
    rpe.patterns.space_filling.sort_by_hilbert(points, 4)
    ...
    assert all(isinstance(p, rpe.math.Vec2) for p in points)
...
def test_a_borrowed_argument_whose_type_is_not_clone():
    ...
    grid = rpe.cfd.grid.MacGrid2(8, 8, 1.0)
    field = rpe.cfd.grid.CellField2(8, 8, 1.0)
    out = rpe.cfd.advection.advect_semi_lagrangian_2d(field, grid, 0.1)
    assert out is not None


def test_an_associated_constant_is_a_class_attribute():
    assert rpe.math.Vec3.ZERO.tolist() == [0.0, 0.0, 0.0]
    assert rpe.units.quantity.Dim.LENGTH.exponents() == [1, 0, 0, 0, 0, 0, 0]


def test_a_reexport_is_reachable_by_its_short_path():
    """`linalg` re-exports `Matrix` and `solve`; the crate's own docs use
    the short names."""
    assert rpe.linalg.Matrix is rpe.linalg.matrix.Matrix
    assert rpe.linalg.solve is rpe.linalg.lu.solve
...
def test_a_submodule_wins_a_name_it_shares_with_a_reexport():
    ...
    assert isinstance(rpe.special.gamma, types.ModuleType)
    assert rpe.special.gamma.gamma(0.5) == pytest.approx(math.sqrt(math.pi))
    assert isinstance(rpe.transforms.fft, types.ModuleType)
    assert rpe.transforms.fft.fft([1.0, 0.0])


def test_an_optional_argument_defaults_to_none():
    sig = rpe.mesh.analyze.self_intersections.__text_signature__
